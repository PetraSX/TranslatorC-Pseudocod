...
public:
	...
	void setPosition(sf::Vector2f pos) {
		...
		float xPos = (pos.x + button.getLocalBounds().width / 3.5) - (text.getLocalBounds().width / 2);
	...
	bool isMouseOver(sf::RenderWindow& window) {
		...
		float buttonXPosWidth = button.getPosition().x + button.getLocalBounds().width;                    //coordonata x a butonului + latimea
